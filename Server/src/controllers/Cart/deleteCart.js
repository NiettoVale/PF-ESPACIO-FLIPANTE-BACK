...
const deleteCart = async (req, res) => {
  try {
    ...
    const cartProducts = user.Products.filter((product) => !product.delete);

    // Ordenar los productos favoritos por ID de forma ascendente
    const cart = cartProducts.sort((a, b) => a.id - b.id);

    const productCart = cart.find((prod) => prod.id === productId);

    await productCart.update({ delete: true });
